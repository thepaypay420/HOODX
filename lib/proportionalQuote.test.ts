...
import {proportionalAbi,quoteProportionalDeposit,quoteProportionalWithdrawal,prepareProportionalDeposit,type ProportionalState} from './proportionalQuote';
const E=10n**18n;
...
function response(name:'BuyQuote'|'WithdrawalQuote',outputs:readonly bigint[],cash=0n) {
  const data=name==='BuyQuote'?encodeErrorResult({abi:proportionalAbi,errorName:name,args:[outputs]}):encodeErrorResult({abi:proportionalAbi,errorName:name,args:[cash,outputs]});
  return new BaseError('call reverted',{cause:new ContractFunctionRevertedError({abi:proportionalAbi,data,functionName:name==='BuyQuote'?'quoteBuys':'quoteWithdrawal'})});
}
