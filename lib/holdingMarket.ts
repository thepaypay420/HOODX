...
export function selectMarket(pairs: Pair[], token: string, now = Date.now()): HoldingMarket {
  ...
  try { const url = new URL(pair?.info?.imageUrl || ""); if (url.protocol === "https:" && url.hostname === "cdn.dexscreener.com" && !url.username && !url.password && !url.port) image = url.href; } catch {}
