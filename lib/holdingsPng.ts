...
export async function renderHoldingsPng({ slug, rows, cash, markets, oldest }: { slug: string; rows: HoldingRow[]; cash?: HoldingRow; markets: Record<string, HoldingMarket | null>; oldest?: number }) {
  ...
  text("#", 48, 222, 16, "#8baba3"); text("TOKEN", 140, 222, 16, "#8baba3"); text("MARKET CAP / USD", 943, 222, 16, "#8baba3", "right"); text("TARGET", 1152, 222, 16, "#8baba3", "right");
  ...
  text("xhoodindex.com", 48, y + 45, 24); text("More together.", 1152, y + 48, 43, "#50d2bf", "right", script);
