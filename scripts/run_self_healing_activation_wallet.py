"""Run the self-healing wallet activation page behind the private RPC relay."""
...
def main() -> int:
    ...
    node = os.environ.get("HOODX_NODE", "node")
    with local_rpc(endpoint) as url:
        env["ROBINHOOD_RPC_URL"] = url
        return subprocess.run(
            [node, "scripts/activate_self_healing_controllers_wallet.mjs"],
            cwd=project,
            env=env,
        ).returncode
