...
contract SuccessorWatchlistForkTest is Test {
 ...
 function run(V2Hop memory assetHop,uint256 amount)internal {address token=assetHop.tokenOut;bool needsBridge=assetHop.tokenIn!=W&&assetHop.tokenIn!=address(0);V2Hop[] memory b=new V2Hop[](needsBridge?2:1);b[b.length-1]=assetHop;if(needsBridge)b[0]=bridge(assetHop.tokenIn);
 uint256 expected=amount;for(uint i;i<b.length;i++)expected=quote(b[i],expected);require(expected>0,"zero buy quote");IERC20(W).forceApprove(address(ex),amount);
...
`;
for(const t of d.assets){let body;const ps=t.pools.filter(p=>p.version==='v4'&&p.identityMatches&&BigInt(p.activeLiquidity)>0n).sort((a,b)=>(b.displayLiquidityUsd||0)-(a.displayLiquidityUsd||0));
if(['PONS','Index','UP','musebook'].includes(t.symbol)){const refs=t.references.filter(r=>r.quote.toLowerCase()==='0x0bd7d308f8e1639fab988df18a8011f41eacad73'&&r.history1800&&BigInt(r.liquidity)>0n).sort((a,b)=>BigInt(a.liquidity)>BigInt(b.liquidity)?-1:1);if(!refs.length)throw Error('No mature WETH route for '+t.symbol);body=`h.kind=3;h.tokenIn=W;h.fee=${refs[0].fee};`;}
else if(t.symbol==='STELX'){const exact=ps.find(p=>p.key.currency0.toLowerCase()==='0x0000000000000000000000000000000000000000'&&p.key.currency1.toLowerCase()===t.token.toLowerCase()&&p.key.fee===0&&p.key.tickSpacing===200&&p.key.hooks.toLowerCase()==='0xe5e702641ea86f4ae6cc3cdaed2b886f976be044');if(!exact)throw Error('Pinned STELX pool unavailable');const k=exact.key;body=`h.kind=4;h.tokenIn=${a(k.currency0)};h.key=V2PoolKey(${a(k.currency0)},${a(k.currency1)},${k.fee},${k.tickSpacing},${a(k.hooks)});`;}
...
else{const r=t.references.find(r=>r.quote.toLowerCase()==='0x0bd7d308f8e1639fab988df18a8011f41eacad73'&&BigInt(r.liquidity)>0n);if(!r)throw Error('No executable route for '+t.symbol);body=`h.kind=3;h.tokenIn=W;h.fee=${r.fee};`;}
for(const [label,size] of [['Small','.0001'],['Medium','.001'],['Large','.005']])s+=`function test${t.symbol.replace(/\W/g,'')}${label}()public{V2Hop memory h;h.tokenOut=${a(t.token)};${body}run(h,${size} ether);}\n`;}
