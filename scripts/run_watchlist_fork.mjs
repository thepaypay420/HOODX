...
const out=fs.openSync(cl?'../outputs/cl-oracle-fork.txt':matured?(process.argv.includes('--net-basket')?'../outputs/net-basket-fork.txt':process.argv.includes('--vault')?'../outputs/matured-vault-fork.txt':'../outputs/matured-oracle-fork.txt'):prices?'../outputs/price-sources-fork.txt':quotron?'../outputs/quotron-router-fork.txt':successor?(diagnostic?'../outputs/successor-diagnostics.txt':'../outputs/successor-watchlist-fork.txt'):alternative?'../outputs/watchlist-alternative-routes.txt':diagnostic?'../outputs/watchlist-route-diagnostics.txt':'../outputs/watchlist-protected-routes.txt','w');
const child=spawn('../work/foundry/forge.exe',['test','--match-contract',cl?'ClOracleForkTest':matured?'MaturedOracleForkTest':prices?'PriceSourcesForkTest':quotron?'QuotronRouterForkTest':successor?'SuccessorWatchlistForkTest':'WatchlistRoutesForkTest','--threads','1','--no-storage-caching',...(cl?['--match-test','testClNet','-vv']:matured?['--match-test',process.argv.includes('--net-basket')?'testMaturedNetBasket':process.argv.includes('--vault')?'testMatured(Basket|ZealV3|Prism)':'testMatured',process.argv.includes('--trace')?'-vvvv':'-vv']:quotron?['--match-test','test(CanonicalTaxedPrism|(CanonicalRouter|IntegratedQuote|IndependentOracle)RoundTrip)','-vvvv']:alternative?['--match-test','testSHROOMAlternative','-vv']:diagnostic?['--match-test',successor?'test(PRISM|NET)':'test(QUOTRON|SHROOM)Small','-vvvv']:['-vv'])],{env:{...process.env,ROBINHOOD_RPC_URL:`http://127.0.0.1:${server.address().port}`,HOODX_FORK_TEST:'true',HOODX_FORK_BLOCK:forkBlock},stdio:['ignore',out,out]});
