...
let forkBlock='0';if(successor){const r=await fetch(endpoint,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({jsonrpc:'2.0',id:1,method:'eth_blockNumber',params:[]})});const j=await r.json();if(!j.result)throw Error('Block unavailable');forkBlock=String(BigInt(j.result));console.log('Successor fork block:',forkBlock);}
const alternative=process.argv.includes('--alternative');
const diagnostic=process.argv.includes('--diagnostic');
const out=fs.openSync(quotron?'../outputs/quotron-router-fork.txt':successor?(diagnostic?'../outputs/successor-diagnostics.txt':'../outputs/successor-watchlist-fork.txt'):alternative?'../outputs/watchlist-alternative-routes.txt':diagnostic?'../outputs/watchlist-route-diagnostics.txt':'../outputs/watchlist-protected-routes.txt','w');
const child=spawn('../work/foundry/forge.exe',['test','--match-contract',quotron?'QuotronRouterForkTest':successor?'SuccessorWatchlistForkTest':'WatchlistRoutesForkTest','--threads','1','--no-storage-caching',...(quotron?['--match-test','testCanonicalRouterRoundTrip','-vvvv']:alternative?['--match-test','testSHROOMAlternative','-vv']:diagnostic?['--match-test',successor?'test(PRISM|NET)':'test(QUOTRON|SHROOM)Small','-vvvv']:['-vv'])],{env:{...process.env,ROBINHOOD_RPC_URL:`http://127.0.0.1:${server.address().port}`,HOODX_FORK_TEST:'true',HOODX_FORK_BLOCK:forkBlock},stdio:['ignore',out,out]});
