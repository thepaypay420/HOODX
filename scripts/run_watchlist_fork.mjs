...
const out=fs.openSync(alternative?'../outputs/watchlist-alternative-routes.txt':diagnostic?'../outputs/watchlist-route-diagnostics.txt':'../outputs/watchlist-protected-routes.txt','w');
const child=spawn('../work/foundry/forge.exe',['test','--match-contract','WatchlistRoutesForkTest','--threads','1','--no-storage-caching',...(alternative?['--match-test','testSHROOMAlternative','-vv']:diagnostic?['--match-test','test(QUOTRON|SHROOM)Small','-vvvv']:['-vv'])],{env:{...process.env,ROBINHOOD_RPC_URL:`http://127.0.0.1:${server.address().port}`,HOODX_FORK_TEST:'true'},stdio:['ignore',out,out]});
