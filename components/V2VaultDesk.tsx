...
import { BaseError, formatEther, parseEther, zeroAddress, type Address } from "viem";
import { robinhood } from "@/lib/chain";
import { publicClient, useWallet } from "@/lib/wallet";
import { preflightV2Routes } from "@/lib/v2Preflight";
import { v2VaultAbi } from "@/lib/v2";

type Snapshot = { account: Address; vault: Address; firstMinimum: bigint; shares: bigint; supply: bigint; paused: boolean; assets?: bigint; tokens: Address[]; claims: { token: Address; amount: bigint }[] };
...
export function V2VaultDesk({ vault, slug }: { vault: Address; slug: string }) {
  ...
  const read = useCallback(async () => {
    ...
    const [shares, supply, paused, tokens, weth, firstMinimum] = await Promise.all([
    ...
    setSnap({ account, vault, firstMinimum, shares, supply, paused, tokens: [...tokens], claims: claimTokens.map((token, i) => ({ token, amount: amounts[i] })) });
    const assets = await publicClient.readContract({ address: vault, abi: v2VaultAbi, functionName: "totalAssets" }).catch(() => undefined);
    if (ticket !== generation.current) return;
    setSnap({ account, vault, firstMinimum, shares, supply, paused, assets, tokens: [...tokens], claims: claimTokens.map((token, i) => ({ token, amount: amounts[i] })) });
  }, [address, vault]);
  ...
  async function transact(action: "deposit" | "withdraw" | "assets" | "claim", token?: Address) {
