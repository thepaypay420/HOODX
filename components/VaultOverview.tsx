...
type Row = { token: Address; symbol: string; weight: number; balance: string; cash: boolean };
const short = (a: string) => `${a.slice(0, 6)}…${a.slice(-4)}`;
const amount = (v?: bigint) => v === undefined ? "—" : Number(formatEther(v)).toLocaleString(undefined, { maximumFractionDigits: 5 });

export function VaultOverview({ vault, slug, shares, assets, supply, paused, connected }: { vault: Address; slug: string; shares?: bigint; assets?: bigint; supply?: bigint; paused?: boolean; connected: boolean }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const rows = await Promise.all([...tokens, weth].map(async token => {
        ...
        return { token, symbol, weight, cash: isCash, balance: decimals === undefined ? "Unavailable" : Number(formatUnits(balance, decimals)).toLocaleString(undefined, { maximumFractionDigits: 5 }) };
      }));
  ...
  return <>
    ...
    <header className="vault-hero desk">
      ...
      <div className="vault-hero-footer"><span className="vault-tag"><i />{paused === undefined ? "Reading vault" : paused ? "Deposits paused" : "Open for deposits"}</span><a href={`https://robin.etherscan.io/address/${vault}`} target="_blank" rel="noreferrer">View contract {short(vault)} ↗</a><span className="vault-version">V2</span></div>
    </header>
    <div className="vault-stats">
      ...
      <div><p>Basket</p><strong>{current ? current.rows.length - 1 : "—"} <small>assets</small></strong><span>Plus a WETH cash reserve</span></div>
      <div><p>Deposit fee</p><strong>{current ? current.fee.toFixed(2) : "—"}<small>%</small></strong><span>Creator + protocol</span></div>
    </div>
    <section className="vault-basket holo"><div className="vault-section-heading"><div><p className="vault-eyebrow">LOOK INSIDE</p><h2>The basket.</h2></div><span className="vault-tag">Live on-chain targets</span></div>
      {current ? <><div className="vault-allocation" aria-label="Target allocation">{current.rows.filter(r => r.weight > 0).map((r, i) => <div key={r.token} title={`${r.symbol}: ${r.weight / 100}%`} style={{ flex: r.weight, background: `hsl(${165 + i * 13} 45% ${45 + i % 3 * 9}%)` }} />)}</div><div className="vault-table-scroll"><table className="vault-table"><thead><tr><th>Asset</th><th>Target</th><th>Vault balance</th></tr></thead><tbody>{current.rows.map((r, i) => <tr key={r.token}><td><a href={`https://robin.etherscan.io/token/${r.token}`} target="_blank" rel="noreferrer"><span className="vault-coin" style={{ color: `hsl(${165 + i * 13} 60% 70%)` }}>{r.symbol.slice(0, 2)}</span><span>{r.symbol}<small>{r.cash ? "Cash reserve" : short(r.token)}</small></span></a></td><td>{(r.weight / 100).toFixed(2)}%<span className="vault-weight"><i style={{ width: `${r.weight / 100}%` }} /></span></td><td>{r.balance}</td></tr>)}</tbody></table></div><p className="vault-footnote">Targets are intended allocations, not current portfolio weights. Balances exclude reserved claims.</p></> : <p className="vault-footnote">{failed ? "Basket data is temporarily unavailable. Transaction and recovery controls remain below." : "Reading the basket from Robinhood Chain…"}</p>}
    </section>
  </>;
}
