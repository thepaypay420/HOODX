...
class HudLivePointerTest(unittest.TestCase):
    ...
    LIVE_FACTORY = "0xB6CdCA31fC5EDAb747d69Ec6D7e6d251De8DE03a"
    SUPERSEDED_FAANGX_FACTORY = "0x67427270Dd489822d6348C8c68f7545B0f3c27eE"
    ...
    def test_config_ignores_legacy_vercel_pins(self):
        ...
        self.assertIn(self.SUPERSEDED_FAANGX_FACTORY, src)
        ...
        # V2 resolves official vaults from the reviewed production registry.
        self.assertIn("productionV2Vault(clean)", client)
        self.assertIn("address: productionV2Factory", client)
        self.assertIn('functionName: "bySlug"', client)
        self.assertNotIn('from "@/lib/config"', client)
        self.assertIn("<V2VaultDesk", client)
    ...
    def test_curator_book_panel_shipped(self):
        ...
        # Legacy curator components remain archived; V2 uses its own gated controls.
        self.assertNotIn("CuratorBalanceSection", client)
        v2 = (here / "components" / "V2VaultDesk.tsx").read_text(encoding="utf-8")
        self.assertIn("snap.owner.toLowerCase()", v2)
        self.assertIn('functionName: "setPaused"', v2)
        self.assertIn('functionName: "emergencyUnwind"', v2)
