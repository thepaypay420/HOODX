...
import {IV2Policy, IV2Executor, IV2Oracle, IV2Weth} from "../v2/Types.sol";

/// @notice New immutable vault implementation. Not an upgrade to historical HOODX clones.
contract HoodxIndexV3 is ERC20, Ownable2Step, ReentrancyGuard {
    ...
    constructor(address policy_, address feeModel_) ERC20("", "") Ownable(address(1)) {
        if (feeModel_ != address(0) && feeModel_.code.length == 0) revert Invalid();
    ...
    function _executionFloor(address token, bool buying, uint256 fairValue) private view returns (uint256) {
        return Math.mulDiv(Math.mulDiv(fairValue, executionFactor(token, buying), 1e18), 9700, BPS);
    }
    ...
    function previewDeposit(uint256 gross) external view returns (uint256) {
        ...
        uint256 expected = net;
        for (uint256 i; i < tokens.length; ++i) {
            uint256 amount = Math.mulDiv(net, targetBps[tokens[i]], BPS);
            if (amount >= MIN_SLEEVE) {
                expected -= amount - Math.mulDiv(amount, executionFactor(tokens[i], true), 1e18);
            }
        }
        return Math.mulDiv(expected, totalSupply() + VIRTUAL_SHARES, totalAssets() + VIRTUAL);
    }

    /// Indicative independent value after modeled execution costs; market movement is not a promise.
    function previewWithdraw(uint256 shares) external view returns (uint256 expected) {
        ...
        for (uint256 i; i < tokens.length; ++i) {
            ...
            if (amount != 0) expected += Math.mulDiv(_value(token, amount), executionFactor(token, false), 1e18);
        }
    }
    ...
    function deposit(uint256 minShares, uint256 deadline) external payable nonReentrant returns (uint256 shares) {
        ...
        for (uint256 i; i < tokens.length; ++i) {
            ...
            uint256 floor = _executionFloor(t, true, Math.mulDiv(amount, unit, px));
            if (floor == 0) continue;
    ...
    function _sell(address token, uint256 amount, uint256 minOut, uint256 deadline) internal returns (uint256) {
        uint256 floor = _executionFloor(token, false, _value(token, amount));
        ...
        nonReentrant
    {
        ...
        if (buy) {
            if (paused) revert Paused();
            uint256 floor = _executionFloor(token, true, Math.mulDiv(amount, 1e18, _value(token, 1e18)));
